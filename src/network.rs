...
impl MemoryNetwork {
    ...
    pub async fn dispatch(&mut self) -> Result<()> {
        while let Some(msg) = self.receiver.recv().await {
            let to_more = msg.to_more.to_owned();
            if msg.to.is_none() && msg.to_more.is_none() {
            ...
            } else if let Some(more) = to_more{
              for sender in more{
                let msg = msg.clone();
                if let Some(h) = self.sender.get(&sender) {
                  h.send(msg).await?;
              }
              }
            }
...
impl FailureNetwork {
    ...
    /// * `_config`:
    pub fn spawn(mut addr: SocketAddr, _config: HashMap<u64, SocketAddr>) -> NetworkAdaptor {
...
impl TcpNetwork {
    pub fn spawn(
        mut addr: SocketAddr,
        mut config: HashMap<u64, SocketAddr>,
    ) -> NetworkAdaptor {
        let (tx, rx) = mpsc::channel(1);
        let (sender_tx, sender_rx) = mpsc::channel(1);
        if !addr.ip().is_loopback() {
    ...
    async fn run(&mut self) -> Result<()> {
      while let Some(pkg) = self.sender_rx.recv().await {
          let to = pkg.to;
          let to_more = pkg.to_more.to_owned();
          let pkg = bincode::serialize(&pkg).unwrap();
          if to.is_none() && to_more.is_none(){
              for addr in self.addrs.values() {
                  self.sender.send(*addr, pkg.clone().into()).await?;
              }
          }else if let Some(more) = to_more{
            for public_key in more{
              if let Some(addr) = self.addrs.get(&public_key){
                self.sender.send(*addr, pkg.clone().into()).await?;
              }
            }
