...
impl Metrics {
    ...
    pub async fn dispatch(&mut self) {
        while let Some((block, block_type, finalized_time)) = self.finalized_block_rx.recv().await {
            ...
            self.decide_delay = self.commit_delay.clone();
            self.commit_delay = self.prepare_delay.clone();
            self.last_block = self.start_time.elapsed().as_micros() as u64 / self.finalized_blocks;
            self.prepare_delay = self.last_block.clone();
...
impl MetricsSample {
    fn from_metrics(m: &Metrics) -> Self {
        ...
        let average_delay = (m.prepare_delay + m.commit_delay + m.decide_delay) as f64 / 1000 as f64;
        Self {
            block_delay,
            e2e_delay,
            average_delay,
