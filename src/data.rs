...
use crate::curve::{hash, Digest};
use anyhow::Result;
use serde::{Deserialize, Serialize};
use tracing::{debug, error, trace};
use k256::{Scalar,AffinePoint};
// use blake3::{hash, Hash};
use crate::{client::FakeClient, mempool::Mempool, nodeconf::NodeConfig};
...
pub struct Proof {
    ...
    pub view: u64,
    pub sig: Option<(AffinePoint, Scalar)>,
}

impl Proof {
    pub(crate) fn new(node: Digest, view: u64, sig: Option<(AffinePoint, Scalar)>) -> Self {
        Self { node, view, sig }
    }
}
...
pub struct Block {
    ...
    pub timestamp: u64,
    pub author: u64,
    // todo: How to add block signature
...
impl BlockTree {
    ...
    pub(crate) fn new_key_block(&mut self, justify: Proof) -> Block {
        ...
        //trace!("latest:{:?}, blocks:{:?}, ",self.latest,self.blocks);
        ...
        // trace!("current:{:?}, latest_key_block:{:?}",current.0.hash(),self.latest_key_block);
        ...
        self.parent_key_block.insert(block.hash(), self.latest_key_block);
    ...
    pub(crate) fn new_in_between_block(&mut self, justify: Proof, lowerbound: usize) -> Option<Block> {
    ...
    pub(crate) fn finalize(&mut self, block: Digest) -> Vec<(Block, BlockType, u64)> {
        ...
        // trace!("height1: {:?}, height2: {:?}",self.blocks.get(&block).unwrap().0.height,self.blocks.get(&self.finalized).unwrap().0.height);
